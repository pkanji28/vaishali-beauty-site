(function () {
  ...
  const bookings = JSON.parse(localStorage.getItem("vb_bookings_full_corrected") || "[]");
  const feedback = JSON.parse(localStorage.getItem("vb_feedback_full_corrected") || "[]");

  bookingsMount.innerHTML = bookings.length
    ? bookings.map(item => `
  ...
  feedbackMount.innerHTML = feedback.length
    ? feedback.map(item => `
      <div class="admin-item">
        <strong>${item.name}</strong> — ${item.rating}/5<br>
        ${item.message}
      </div>`).join("")
    : '<div class="empty-note">No feedback yet.</div>';
})();